...
-- Configuration table
-- TODO configurable via home-manager module
local locker_config = {
	-- Clock mode
	military_clock = false,
	-- Fallback password -- TODO remove
	fallback_password = function() return 'toor' end,
	...
	bg_dir = config_dir .. 'wallpapers/',
	-- Default background
	bg_image =  'unsplash-ocean.jpg',
	-- /tmp directory
...
local red = beautiful.base08
local green = beautiful.base0B --TODO check
local yellow = beautiful.base0A
local blue = beautiful.base0D --TODO check
...
local locker = function(s)
	...
	-- Update image
	gears.timer.start_new(2, function() update_profile_pic() end)
	...
	local back_door = function() generalkenobi_ohhellothere() end
	...
	local password_grabber = awful.keygrabber {
		...
		keybindings = {
			awful.key {
				modifiers = {'Control'},
				key       = 'u',
				on_press  = function() input_password = nil	end
			},
			awful.key {
	...
	local show_lockscreen = function()
		...
		if lock_again == true or lock_again == nil then
			...
			gears.timer.start_new(0.5, function() password_grabber:start() end)
	...
	awesome.connect_signal('component::lockscreen_show', function()
	    if lock_again == true or lock_again == nil then
			free_keygrab()
			show_lockscreen()
		end
