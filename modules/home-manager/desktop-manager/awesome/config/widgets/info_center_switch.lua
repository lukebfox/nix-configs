...
local notif_button = clickable_container(
	wibox.container.margin(notif_text, dpi(0), dpi(0), dpi(7), dpi(7))
)
...
local wrap_notif = wibox.widget {
    ...
    shape        = function(cr, width, height) gears.shape.partially_rounded_rect(cr, width, height, false, true, true, false, beautiful.groups_radius) end,
    notif_button,
}
...
local today_button = clickable_container(
	wibox.container.margin(today_text, dpi(0), dpi(0), dpi(7), dpi(7))
)
...
local wrap_today = wibox.widget {
    ...
    border_width = dpi(1),
    border_color = beautiful.groups_title_bg,
    shape        = function(cr, width, height) gears.shape.partially_rounded_rect(cr, width, height, true, false, false, true, beautiful.groups_radius) end,
    today_button
}
...
notif_button:buttons(gears.table.join(awful.button({}, 1, nil, function() switch_rdb_pane('notif_mode') end)))

today_button:buttons(gears.table.join(awful.button({}, 1, nil, function() switch_rdb_pane('today_mode') end)))
