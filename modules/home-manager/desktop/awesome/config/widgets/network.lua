...
-- Configuration TODO make configurable in nix?
local interfaces = {
	wlan_interface = 'wlo1', -- FIXME these are hardcoded for my laptop,
	lan_interface  = 'eno1'  --       so they won't work for cloud machines
}
...
local return_button = function()
	...
	widget_button:buttons(
		gears.table.join(
			awful.button({}, 1, nil, function() awful.spawn(apps.default.network_manager, false) end)
		)
	)
	...
	local update_startup = function() if startup then startup = false end end

	-- Consider reconnecting a startup
	local update_reconnect_startup = function(status) reconnect_startup = status end

	local update_tooltip = function(message) network_tooltip:set_markup(message) end
			...
			awful.spawn.easy_async_with_shell(
				...
				function(stdout)
					...
					if not stdout:match('Connected but no internet') then
						...
						icon = icons['wifi_strength_' .. tostring(strength)]
						update_wireless_data(wifi_strength_rounded, true)
					else
						icon = icons['wifi_strength_' .. tostring(strength) .. '-alert']
	...
	local update_wired = function()
		...
		local notify_connected = function()
			...
			local title = 'Connection Established'
			local app_name = 'System Notification'
			network_notify(message, title, app_name, icons.wired)
		end
	...
	local check_network_mode = function()
		awful.spawn.easy_async_with_shell(
			...
			function(stdout)
				...
				if stdout:match('No internet connection') then update_disconnected()
				elseif stdout:match('wireless')           then update_wireless()
				elseif stdout:match('wired')              then update_wired()
				end
			end
		)
	end
	...
	local network_updater = gears.timer {
		...
		autostart = true,
		call_now = true,
		callback = function() check_network_mode() end
	}
