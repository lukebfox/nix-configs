...
local check_if_playing = function()
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			if not (stdout == nil or stdout == '') then
				play_button_img:set_image(icons.pause)
				update_volume_slider()
			else
				play_button_img:set_image(icons.play)
			end
		end
	)
end
	...
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			if stdout:match('on') then
				repeat_button_img:set_image(icons.repeat_on)
			else
				repeat_button_img:set_image(icons.repeat_off)
			end
		end
	)
end
...
local check_random_status = function()
	awful.spawn.easy_async_with_shell(
		...
		function(stdout)
			...
			if stdout:match("on") then
				random_button_image:set_image(icons.random_on)
			else
				random_button_image:set_image(icons.random_off)
			end
		end
	)
end

vol_slider.vol_slider:connect_signal('property::value', function()
	awful.spawn.easy_async_with_shell(
		'mpc volume ' .. vol_slider.vol_slider:get_value(), function() end
	)
end)
