...
local calculator_screen = wibox.widget {
	...
	margins = dpi(5),
	{
		...
		id = 'calcu_screen',
		text = '0',
		font = 'Inter Regular 20', --TODO font
		align = 'right',
		valign = 'center'
	}
}
...
local calculate = function ()
	...
	if string_expression:sub(-1):match('[%+%-%/%*%^%.]') then return end

	local func = assert(load('return ' .. string_expression))
	local ans = tostring(func())

	-- Convert -nan to undefined
	if ans == '-nan' then calcu_screen:set_text('undefined') return	end
...
-- Clear screen
local clear_screen = function() calculator_screen.calcu_screen:set_text('0') end
...
local format_screen = function(value)
	...
	elseif calcu_screen:get_text() == 'inf'
		or calcu_screen:get_text() == 'undefined'
		or calcu_screen:get_text() == 'SYNTAX ERROR' then
...
kb_button:buttons(
	gears.table.join(
		awful.button({}, 1, nil, function()	toggle_btn_keygrab() end)
	)
)
...
calculator_body:connect_signal('mouse::enter', function() calcu_keygrabber:start() end)
calculator_body:connect_signal('mouse::leave', function() calcu_keygrabber:stop() end)

awesome.connect_signal('widget::calc_start_keygrab', function()	calcu_keygrabber:start() end)
awesome.connect_signal('widget::calc_stop_keygrab', function() calcu_keygrabber:stop() end)
