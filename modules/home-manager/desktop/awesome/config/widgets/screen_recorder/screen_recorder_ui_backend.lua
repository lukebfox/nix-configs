...
local screen_rec_backend = require('widgets.screen_recorder.screen_recorder_scripts')
local screen_rec_ui      = require('widgets.screen_recorder.screen_recorder_ui')
...
sr_close_button:buttons(
	gears.table.join(
		awful.button({}, 1, nil, function()	screen_rec_close() end)
	)
)
...
local screen_close_on_rmb = function(widget)
	widget:buttons(
		gears.table.join(
			awful.button({}, 3,	nil, function() screen_rec_close() end)
		)
	)
end
...
local sr_recording_start = function()
	...
	-- Manipulate UIs
	sr_toggle_imgbox:set_image(icons.recording_button)
	sr_main_imgbox:set_image(icons.recorder_on)
...
local sr_recording_stop = function()
	...
	-- Manipulate UIs
	sr_toggle_imgbox:set_image(icons.start_recording_button)
	sr_main_imgbox:set_image(icons.recorder_off)
	sr_stop_recording()
end

awesome.connect_signal('widget::screen_recorder', function() sr_recording_stop() end)
	...
	countdown_timer = gears.timer.start_new(
		...
		function()
			if seconds == 0 then
				...
				sr_main_imgbox:set_image(icons.recorder_countdown)
...
local status_checker = function()
	if status_recording and not status_countdown then
		sr_recording_stop() -- Stop recording
		return
	elseif not status_recording and status_countdown then
		sr_countdown_stop() -- Stop timer
		return
	end
...
sr_main_button:buttons(
	gears.table.join(
		awful.button({}, 1, nil, function() status_checker() end)
	)
)
